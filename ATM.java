...
public class ATM extends JFrame {
	...
	// Add a text field for the user to input the withdrawal amount
JTextField withdrawalAmountField = new JTextField(10);

// Modify GUI setup to include this text field
bPConst.gridx = 1;
bPConst.gridy = 4; 
buttonPanel.add(withdrawalAmountField, bPConst);

// Modify the withdrawal action listener to use dynamic input
withDraw1.addActionListener(new ActionListener() {
    public void actionPerformed(ActionEvent event) {
        String inputAmount = withdrawalAmountField.getText(); // Get input from the field
        try {
            double withdrawAmount = Double.parseDouble(inputAmount);

            // Check if the balance is sufficient for the withdrawal
            if (balance >= withdrawAmount) {
                balance -= withdrawAmount;
                displayArea.setText("<html>\u00A3" + withdrawAmount + " Withdrawn! <br><br>" + finishedTransaction() + "</html>");
                readyToEnter = false;
                System.out.println("User Has Withdrawn \u00A3" + withdrawAmount);
                updateTransactionHist("User Has Withdrawn \u00A3" + withdrawAmount);
            } else {
                displayArea.setText("<html> Your Balance is below \u00A3" + withdrawAmount + ". Unable to Withdraw!! <br><br>" 
                    + finishedTransaction() + "</html>");
            }
        } catch (NumberFormatException e) {
            displayArea.setText("<html> Invalid amount entered. Please enter a valid number. <br><br>" 
                + finishedTransaction() + "</html>");
        }
    }
});
